...
public class DisplayFour implements StatementCreator, MouseListener {
    ...
    public DisplayFour() throws SQLException {
        ...
        JTextField finalLocationName1 = locationName;
        JTextField finalLocationSize1 = locationSize;
        JTextField finalLocationType1 = locationType;
        deleteLocation.addActionListener(e -> {
            try {
                delete();
                removeTextWhenClicked(finalLocationID2,
                        finalLocationName1, finalLocationSize1, finalLocationType1);
            } catch (SQLException ex) {
                throw new RuntimeException(ex);
            }
        });{
    ...
    private void removeTextWhenClicked(JTextField finalLocationID2, JTextField finalLocationName1,
                                       JTextField finalLocationSize1, JTextField finalLocationType1) throws SQLException {
        finalLocationID2.setText("");
        finalLocationName1.setText("");
        finalLocationSize1.setText("");
        finalLocationType1.setText("");
        refreshJTable(model);
    }
    ...
    //Need to delete text from the text fields
    @Override
    public void delete() throws SQLException {

        PreparedStatement ps = m_dbConn.prepareStatement("DELETE FROM Location WHERE LOC_ID  = ?");
        ps.setInt(1, Integer.parseInt(String.valueOf(this.locationID.getText())));
