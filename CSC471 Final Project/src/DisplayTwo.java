...
public class DisplayTwo implements StatementCreator, MouseListener
{
	 ...
	 */
	private static final String INSERT_SP = 
			"CREATE PROCEDURE add_player(login int, password varchar(10), email varchar(50)) BEGIN "
			+ "INSERT INTO Player (P_login, P_Password, P_email) VALUES (login, password, email); END //";
	/**
	 * Stored Procedure for update
	 */
	private static final String UPDATE_SP =
			"CREATE PROCEDURE update_player(login int, email varchar(50)) BEGIN "
			+ "UPDATE Player SET P_email = email WHERE P_login = login; END //";
	/**
	 * Stored Procedure for delete
	 */
	private static final String DELETE_SP =
			"CREATE PROCEDURE delete_player(login int) BEGIN "
			+ "DELETE FROM Player WHERE (P_Login = login); END //";
	/**
	 ...
	 * @throws SQLException 
	 */
	public DisplayTwo() throws SQLException
	{
	...
	{
		String insertStmt = "CALL add_player(?, ?, ?);";
	...
	{
		String updateStmt = "CALL update_player(?, ?);";
		PreparedStatement ps = m_dbConn.prepareStatement(updateStmt);
		ps.setInt(1, playerToBeEdited);
		ps.setString(2, editToThisEmail);
	...
	{	
		//deletes player
		String deleteStmt = "CALL delete_player(?);";
		PreparedStatement ps = m_dbConn.prepareStatement(deleteStmt);
		ps.setInt(1, deleteThisID);
		ps.executeUpdate();
		
		//deletes characters associated with player
		String deleteCharStmt = "DELETE FROM CharInfo WHERE Player_ID = ?";
		PreparedStatement ps1 = m_dbConn.prepareStatement(deleteCharStmt);
		ps.setInt(1, deleteThisID);
		ps.executeUpdate();
